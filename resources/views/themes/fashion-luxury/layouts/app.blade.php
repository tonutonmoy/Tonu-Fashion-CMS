...
<body class="theme-body theme-{{ $activeTheme ?? 'fashion-luxury' }} theme-mode-{{ $colorMode ?? 'light' }} theme-header-layout-{{ $themeSettings->header_style ?? 'default' }} @if(!empty($hasHomeHero)) has-home-hero @endif @if(request()->boolean('preview')) is-builder-preview @endif" data-theme="{{ $activeTheme ?? 'fashion-luxury' }}" data-color-mode="{{ $colorMode ?? 'light' }}">
